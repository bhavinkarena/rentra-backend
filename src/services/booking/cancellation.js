...
async function estimate(tx, order, value) {
  ...
  const allocations=await tx`SELECT a.*,t.provider,t.environment,t.mode,t.currency,t.provider_payment_id,
    ...
    WHERE a.booking_id IN ${tx(value.visitIds)} AND t.kind='capture' AND t.outcome='succeeded' AND t.verified_at IS NOT NULL ORDER BY a.id`;
  const plans=[];
  for(const visit of visits) {
    ...
    if(!sources.length || sources.some(a=>a.provider!=='razorpay'||a.environment!=='test'||a.mode!=='real')) throw new CheckoutError('PAYMENT_UNSUPPORTED');
    const remaining={rent:entitlement.rent,fee:entitlement.fee,deposit:entitlement.deposit};
    const refunds=sources.map(a=>{
...
export async function commitCancellation(database,session,input,env=process.env) {
  ...
  return ownedCheckout(database,session,value.orderId,async(tx,order)=>{
    ...
    const id=randomUUID(),refundIds=[];
    const groups=new Map();
    for(const visit of preview.visits) for(const a of visit.refunds) {
    ...
    for(const [transactionId,allocations] of groups) {
      ...
      await tx`INSERT INTO refund(id,transaction_id,reference,provider,environment,mode,currency,expected_minor,reason,idempotency_key,request_hash)
        ...
        'Customer visit cancellation',${id},${requestHash})`;
